...
async def reranker_search(
    ...
    rerank_results = await reranker._make_rerank_request(
        query, doc_texts, instruction=reranker_instruction
    )

    sorted_results = [
        (results[item["index"]][0], item["relevance_score"])
                ...
                async with sem:
                    ...
                    if config.use_reranker:
                        if config.use_emb:
                            results = await search_with_emb_index(
                                query=question, emb_index=emb_index, top_n=100
                            )
                        else:
                            results = await asyncio.to_thread(
                                search_with_bm25_index, question, bm25, docs, 100
                            )
                    ...
                    if top_results:
                        ...
                        for doc, score in top_results:
                            # 优先使用event_log格式化文本（如果存在）
                            if doc.get("event_log") and doc["event_log"].get(
                                "atomic_fact"
                            ):
                                event_log = doc["event_log"]
