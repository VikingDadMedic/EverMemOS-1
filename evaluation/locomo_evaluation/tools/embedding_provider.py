    ...
    def embed(self, texts: List[str]) -> str:
        if 'Qwen3' in self.model_name:
            response = requests.post(
                self.base_url, json={"input": texts, "model": self.model_name}
            ).json()
        else:
            raise ValueError(
...
if __name__ == "__main__":
    inputs = ["Tom上个月从他家乡搬过来了", "Frank的家乡是瑞士"]

    reranker = EmbeddingProvider(
        base_url="http://0.0.0.0:11000/v1/embeddings", model_name="Qwen3-Embedding-4B"
    )
