...
class RerankerProvider:
    ...
    def rerank(
        self, queries: List[str], docs: List[str], instruction: str = None
    ) -> str:
        if 'Qwen3' in self.model_name:
            ...
            raise ValueError(
                f"Model {self.model_name} is not supported, only Qwen3-Reranker series models is supported"
            )
        ...
        documents = [document_template.format(doc=doc, suffix=suffix) for doc in docs]
...
if __name__ == "__main__":
    ...
    reranker = RerankerProvider(
        base_url="http://0.0.0.0:12000/score", model_name="Qwen3-Reranker-4B"
    )
