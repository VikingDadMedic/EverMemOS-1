...
from ...prompts.en.profile_mem_part1_prompts import (
    CONVERSATION_PROFILE_PART1_EXTRACTION_PROMPT,
)
from ...prompts.en.profile_mem_part2_prompts import (
    CONVERSATION_PROFILE_PART2_EXTRACTION_PROMPT,
)
from ...prompts.en.profile_mem_part3_prompts import (
...
class ProfileMemoryExtractor(MemoryExtractor):
    ...
    async def extract_memory(
        ...
        if not combined_user_ids:
            logger.warning(
                "[ProfileMemoryExtractor] No valid user_ids found in combined results"
            )
            return None
        ...
        for user_id in combined_user_ids:
            ...
            if user_id in part2_map:
                ...
                if "projects_participated" in part2_profile:
                    combined_profile["projects_participated"] = part2_profile[
        ...
        for profile in merged_profiles:
            if (
                profile.user_id == "5fc48d31db1c7c1701161e34"
                and profile.group_id == "676a2d05d4dd26352df7fad3"
            ):
                logger.debug("[ProfileMemoryExtractor] Debug:  profile=%s", profile)
    ...
    def _annotate_relative_dates(self, response: str) -> str:
        ...
        return annotate_relative_dates(
            response, default_date=self._default_evidence_date
        )
    ...
    def _parse_profile_response_payload(response: str) -> Dict[str, Any]:
        ...
        Also normalizes smart quotes to improve robustness.
        """
        if not response:
        ...
        json_match = re.search(
            r"```(?:json)?\s*(\{[\s\S]*?\})\s*```", sanitized, re.DOTALL
        )
        if json_match:
        ...
        # Fallback: literal_eval then json
        try:
            ...
            raise ValueError(f"unable to parse JSON payload: {exc}")
    ...
    def _merge_single_profile(
        cls, existing: ProfileMemory, new: ProfileMemory, group_id: str
    ) -> ProfileMemory:
    ...
    def _extract_group_important_info(
        cls, memcells: List[MemCell], group_id: str
    ) -> Dict[str, Any]:
    ...
    def _convert_important_info_to_evidence(
        cls, important_info: Dict[str, Any]
