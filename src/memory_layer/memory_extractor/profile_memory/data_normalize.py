        ...
        if extract_evidences:
            ...
            for evidence in evidence_items:
                ...
                if valid_conversation_ids is not None:
                    ...
                    if (
                        not conversation_id
                        or conversation_id not in valid_conversation_ids
                    ):
                        logger.warning(
                            "[ProfileMemoryExtractor] LLM Generated Unknown Conversation ID %s",
                            conversation_id,
...
def merge_skill_lists(
    ...
    return [
        {
            "skill": skill,
            "level": merged_map[skill]["level"],
            "evidences": merged_map[skill]["evidences"],
        }
        for skill in order
    ]
...
def accumulate_old_memory_entry(
    ...
    try:
        ...
        logger.error(
            "[ProfileMemoryExtractor] Failed to extract old memory entry: %s", exc
        )
...
def merge_single_profile(
    ...
    merged_motivation = merge_value_with_evidences_lists(
        existing.motivation_system, new.motivation_system
    )
    merged_fear = merge_value_with_evidences_lists(
        existing.fear_system, new.fear_system
    )
    merged_value_system = merge_value_with_evidences_lists(
        existing.value_system, new.value_system
    )
    ...
    merged_colloquialism = merge_value_with_evidences_lists(
        existing.colloquialism, new.colloquialism
    )
    merged_way_of_decision = merge_value_with_evidences_lists(
        existing.way_of_decision_making, new.way_of_decision_making
    )
    merged_personality = merge_value_with_evidences_lists(
        existing.personality, new.personality
    )
    merged_user_goal = merge_value_with_evidences_lists(
        existing.user_goal, new.user_goal
    )
    merged_working_habits = merge_value_with_evidences_lists(
        existing.working_habit_preference, new.working_habit_preference
    )
    merged_interests = merge_value_with_evidences_lists(
        existing.interests, new.interests
    )
    ...
    merged_work_responsibility = merge_value_with_evidences_lists(
        existing.work_responsibility, new.work_responsibility
    )
