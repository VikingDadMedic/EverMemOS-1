...
faiss_config = {"dim": 768, "nlist": 1000, "m": 512, "nbits": 8, "nprobe": 20}
...
class FaissIVFPQ:
    ...
    def train(self, data_generator, total_samples: int, batch_size: int = 100000):
        ...
        if True:
            print(
                f"Data size {total_samples} < 1000, using IndexFlatIP (no training needed)"
            )
    ...
    def add_vectors_with_metadata(
        ...
        for batch in tqdm(data_generator, desc="Adding vectors and metadata"):
            ...
            if len(vectors) != len(metadatas):
                raise ValueError(
                    f"Vector count({len(vectors)}) doesn't match metadata count({len(metadatas)})"
                )

            ids = np.arange(
                self.current_id, self.current_id + len(vectors), dtype=np.int64
            )
            ...
            if custom_ids is not None:
                ...
                for faiss_id, chunk_id in zip(
                    ids, custom_ids[added_count : added_count + len(vectors)]
                ):
        ...
        print(
            f"Vector and metadata addition completed, index now contains {self.index.ntotal} vectors"
        )
    ...
    def load_index(self, index_path: str, metadata_path: Optional[str] = None):
        ...
        with open(metadata_path, 'rb') as f:
            ...
            # Restore index type
            self.index_type = data.get(
                'index_type', 'ivfpq'
            )  # Default to ivfpq for backward compatibility
        ...
        print(
            f"Metadata loaded from {metadata_path}, contains {len(self.metadata)} records"
        )
        if 'faiss_params' in data:
            print(
                f"Faiss parameters restored: dim={self.dim}, nlist={self.nlist}, m={self.m}, nbits={self.nbits}, nprobe={self.nprobe}, use_gpu={self.use_gpu}"
            )
        if 'chunk_id_mapping' in data:
            print(
                f"Chunk ID mapping restored, contains {len(self.chunk_id_mapping)} mappings"
            )
...
def evaluate_retrieval_system(
    system, query_vectors: np.ndarray, ground_truth: np.ndarray, k: int = 10
) -> float:
    """
