...
class VectorSearch:
    def __init__(
        ...
        if output_dir is not None:
            self.storage_dir = (
                Path(output_dir) / f"{version}_{self.embedding_model_name}"
            )
        else:
            self.storage_dir = Path(
                f"memory_base/static_memory_base/vector_search/{version}_{self.embedding_model_name}"
            )
    ...
    def _init_embedding_model(self):
        ...
        logger.info(
            f"Initialized embedding model: {self.global_config.embedding_model_name}"
        )
        ...
        for tokens in tqdm(unique_references_tokens):
            if len(tokens) < self.max_tokens_len:
                ...
                for i in range(0, len(tokens), self.max_tokens_len):
                    token2str = self.tokenizer.decode(
                        tokens[i : i + self.max_tokens_len]
                    )
                    # Generate hash ID for this chunk
                    chunk_hash = compute_mdhash_id(token2str)
                    chunk_id = f"chunk-{chunk_hash}"
    ...
    def generate_embeddings(self, chunks):
        ...
        try:
            ...
            if isinstance(chunks, OrderedDict):
                ...
                chunk_ids = [
                    f"chunk-{compute_mdhash_id(content)}" for content in chunks
                ]
        ...
        df = pd.DataFrame(
            {
                ...
                'embedding': list(
                    embeddings
                ),  # Convert numpy arrays to list for parquet storage
            }
        )
    ...
    def train_faiss_index(self, documents):
        ...
        if isinstance(self.faiss_index, faiss.IndexFlatIP):
            ...
            self.metadata = {
                i: metadata_with_ids[i] for i in range(len(metadata_with_ids))
            }
        else:
            ...
            data_generator = generate_data_chunks(embeddings, metadata_with_ids)
            self.faiss_index.add_vectors_with_metadata(
                data_generator, total_samples=num_vectors, custom_ids=hash_ids
            )
    ...
    def add_to_existing_index(self, documents):
        ...
        self.faiss_index.add_vectors_with_metadata(
            data_generator, total_samples=len(embeddings), custom_ids=hash_ids
        )
    ...
    def search(self, query, top_k=10):
        ...
        if not hasattr(self, 'faiss_index') or self.faiss_index is None:
            raise ValueError(
                "Faiss index not initialized. Please train or load an index first."
            )
        ...
        if isinstance(self.faiss_index, faiss.IndexFlatIP):
            ...
            search_results = self.faiss_index.search_with_metadata(
            ...
            for result in search_results[
                0
            ]:  # search_results[0] contains results for the single query
    ...
    def get_all_id_to_rows(self):
        ...
        if not hasattr(self, 'chunk_data') or self.chunk_data is None:
            ...
            chunk_refs_path = os.path.join(
                self.chunk_folder_path, 'chunk_references_str.pkl'
            )
