...
class KeywordSearch:
    ...
    def __init__(
        self, version: str = "default", output_dir=None, k1=1.2, b=0.75, user_id=None
    ):
        """
    ...
    async def search(
        ...
        if not self.bm25 or not self.documents:
            logger.warning(
                "No BM25 index or documents available. Returning empty results."
            )
            return [], []
            ...
            for i, doc in self.documents.items():
                ...
                """,
                    [
                        ...
                        self.user_id
                        or 'default',  # user_name (same as user_id for now)
        ...
        try:
            ...
            try:
                ...
                if isinstance(documents_data, dict):
                    ...
                    logger.error(
                        f"Unexpected documents data type: {type(documents_data)}"
                    )
                    self.documents = {}
            except Exception as e2:
                logger.error(
                    f"Failed to load documents from {self.documents_path}: {e2}"
                )
                self.documents = {}

        logger.info(
            f"BM25 index loaded from {self.index_path} and {len(self.documents)} documents loaded from {self.documents_path}"
        )
