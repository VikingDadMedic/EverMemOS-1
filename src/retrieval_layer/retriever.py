...
class DynamicMemoryRetriever(BaseRetriever):
    ...
    async def initialize(self):
        ...
        try:
            ...
            if Path(self.data_base).is_absolute():
                memory_base_path = (
                    Path(self.data_base) / "dynamic_memory_base" / "memory"
                )
            else:
                ...
                memory_base_path = (
                    current_dir / self.data_base / "dynamic_memory_base" / "memory"
                )
            ...
            if not index_dir.exists():
                ...
                logger.info(
                    "You may need to call build_bm25_idx() first on DynamicMemoryStorageV2"
                )
                self.available_users = set()
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "NLTK data not available, using fallback tokenization"
                    )
                    self.stop_words = set()
                    self.stemmer = None
                    self.nltk_available = False
            ...
            logger.info(
                f"Initialized BM25 retriever with users: {list(self.available_users)}"
            )
            self._initialized = True
    ...
    async def search(
        self, query: str, top_k: int = 10, owner_id: str = "default"
    ) -> List[str]:
        """
                ...
                try:
                    ...
                    for episode in episodes:
                        ...
                        if hasattr(episode, 'summary'):  # Nemori Episode object
                            content = (
                                episode.summary if episode.summary else episode.content
                            )
                            if episode.search_keywords:
                                content += " " + " ".join(episode.search_keywords)
                        else:  # Dict format
                            content = episode.get("summary", "") or episode.get(
                                "content", ""
                            )
                            keywords = episode.get("keywords", []) or episode.get(
                                "search_keywords", []
                            )
                            if keywords:
                    ...
                    logger.error(
                        f"Failed to build BM25 index on-the-fly for {owner_id}: {e}"
                    )
                    return []
...
class KeywordRetriever(BaseRetriever):
    ...
    def __init__(
        self,
        version: str = "default",
        data_base: str = "memory_base/static_memory_base/bm25_index",
        user_id: str = None,
    ):
        """
    ...
    async def initialize(self):
        ...
        try:
            # 使用传入的版本名和 user_id
            self.client = KeywordSearch(
                version=self.version, output_dir=self.data_base, user_id=self.user_id
            )
            # 异步初始化，加载已存在的索引
    ...
    async def search(
        ...
        try:
            logging.info(
                f"KeywordRetriever searching for: '{query}' with top_k={top_k}"
            )
...
class VectorRetriever(BaseRetriever):
    ...
    def __init__(
        self,
        version: str = "default",
        data_base: str = "memory_base/static_memory_base/vector_search",
    ):
        """
...
class MixedRetriever(BaseRetriever):
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.keyword_retriever = KeywordRetriever(
                version=self.version, data_base=self.data_base, user_id=self.user_id
            )
            ...
            self.vector_retriever = VectorRetriever(
                ...
                data_base=os.path.join(
                    self.data_base, "static_memory_base", "vector_search"
                ),
            )
            await self.vector_retriever.initialize()

            # Initialize dynamic memory retriever
            self.dynamic_memory_retriever = DynamicMemoryRetriever(
                version=self.version, data_base=self.data_base
            )
    ...
    async def search(
        ...
        try:
            ...
            dynamic_memory_task = self.dynamic_memory_retriever.search(
                query, top_k, owner_id
            )

            # Wait for all to complete
            keyword_results, vector_results, dynamic_memory_results = (
    ...
    @classmethod
    def rrf_fusion(
        cls, all_results: Dict[str, List[str]], top_k: int = 10
    ) -> List[str]:
        """
...
class UnifiedRetriever:
    ...
    async def initialize(self):
        ...
        try:
            if self.method == "keyword":
                ...
                self.retriever = DynamicMemoryRetriever(
                    version=self.version, data_base=self.data_base
                )
    ...
    async def retrieve(
        self, query: str, top_k: Optional[int] = None, owner_id: str = "default"
    ) -> List[str]:
        """
