...
class RetrieveMemServiceImpl(RetrieveMemServiceInterface):
    ...
    def __init__(self):
        ...
        self._initialized_methods = (
            set()
        )  # 已初始化的方法集合，存储 (method, user_id) 元组
    ...
    async def _get_retriever(self, method: str, data_base: str, user_id: str = None):
        ...
        if cache_key not in self._initialized_methods:
            try:
                ...
                logger.debug(
                    f"Retriever for method '{method}' and user_id '{user_id}' initialized and cached"
                )

            except Exception as e:
                logger.error(
                    f"Failed to initialize retriever for method '{method}': {e}"
                )
                raise
    ...
    async def retrieve(
        ...
        try:
            # 设置数据基础路径
            data_base = (
                "/home/gongjie/b001-memsys/memory_base/dynamic_memory_base/storages/"
            )
            ...
            results = await retriever.retrieve(
                query=query, top_k=top_k, owner_id=user_id
            )

            logger.debug(
                f"RetrieveMemService returned {len(results)} results for method '{method}' and query: {query}"
            )
            return results
...
async def retrieve_memories(
    query: str, method: str = "keyword", top_k: int = 40, user_id: str = "default"
) -> List[str]:
    """
    便捷函数：根据查询字符串检索记忆
